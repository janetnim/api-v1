...
class MakeRequest(Resource):
	def post(self):
		...
		cur.execute("INSERT INTO requests (request, department, status)VALUES('{}','{}','Pending')".format(request, department))
...
class ModifyRequest(Resource):
	def put(self, request_id):
		...
		cur.execute("UPDATE requests SET request={} WHERE request_id={}".format(new_request, request_id));
		cur.execute("SELECT * FROM requests WHERE request_id={}".format(request_id))
		conn.commit()
		items= cur.fetchone()
