...
class User(object):
	...
	def make_request(self, request_id, request, department, result):
	...
	def view_a_request(self, request1, request_data):
		'''lets the user view a specific request using the request's id'''
		request1 = [request1 for request1 in User().request_data if request1["request_id"]==request_id]
		if request1 not in request_data:
			return "No such request present"
		return User().request_data([request1[0]])

	def view_all_requests(self, request_data):
	...
	def modify_request(self, request1, request_data):
		'''checks for existence of a request through its id before modifying it'''
		request1 = [request1 for request1 in User().request_data if request1["request_id"]==request_id]
		if request1 not in request_data:
	...
	def delete_request(self, request1, request_data):
