...
class TestApis(unittest.TestCase):
    ...
    def test_user_make_request(self):
        ...
        response = self.checker.post(
            '/api/v1/request', data=json.dumps(request_data), content_type='application/json')
    ...
    def test_user_view_a_request(self):
        ...
        response = self.checker.get(
            '/api/v1/request/1234', data=json.dumps(request_data), content_type='application/json')
        self.assertEqual(response.status_code, 200)

    def test_user_view_all_requests(self):
        # testing user view all his requests -get
        response = self.checker.get(
            '/api/v1/request', content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_receive_request(self):
        '''testing admin received requests'''
        response = self.checker.get(
